...
//
__global__ void sobelY_3x3(unsigned char *in_img,
  ...
  sobel_m.col_nb(scol_nb);
  sobel_m.row_nb(srow_nb);
    ...
    for (unsigned int sr = 0; sr < srow_nb; ++sr) {
      for (unsigned int sc = 0; sc < scol_nb; ++sc) {
        ...
        int index = (col - sc) * bytes_per_pixel +
                    (row - sr) * per_scanline;
        float scell;
        sobel_m.get(sr, sc, scell);
        g += scell * in_img[index + rgb_offset];
      }
    }
...
int main() {
  ...
  for (int i = 0; i < nb_streams; ++i) {
    cudaimproc::
        sobelY_3x3<<<streamSize / threads_per_block,
